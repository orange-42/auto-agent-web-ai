...
class ContextManager {
     ...
     * @returns 压缩后的消息数组
     */
    compressMessages(messages, loopCount, currentPhaseIndex, displayPhases) {
        // 更早启动压缩，避免在规划/编码阶段把上下文拖到超长后才处理
        if (loopCount < 6 && messages.length < 14)
            return messages;
        ...
        return messages.map((m, i) => {
            ...
            if (i < preservedSystemCount || isFirstUser || i > messages.length - preservedRecentCount)
                return m;
            if (m.role === 'assistant' && Array.isArray(m.tool_calls) && m.tool_calls.length > 0) {
                const toolNames = m.tool_calls
                    .map((tool) => { var _a; return (_a = tool === null || tool === void 0 ? void 0 : tool.function) === null || _a === void 0 ? void 0 : _a.name; })
                    .filter(Boolean)
                    .join(', ');
                return Object.assign(Object.assign({}, m), { content: `[Summary: 该轮已调用工具 ${toolNames || 'unknown'} 推进任务]` });
            }
            if (m.role === 'user' && typeof m.content === 'string') {
                if (/^请继续。/.test(m.content)) {
                    return Object.assign(Object.assign({}, m), { content: '[Summary: 系统已要求模型停止重复并直接给出 JSON 或执行下一步工具]' });
                }
                if (m.content.includes('编码阶段尚未真正写入任何文件')) {
                    return Object.assign(Object.assign({}, m), { content: '[Summary: 系统已强制要求编码阶段先真实写入文件，再允许结束]' });
                }
                if (m.content.includes('reasoning 字段仍是英文')) {
                    return Object.assign(Object.assign({}, m), { content: '[Summary: 系统已要求将 reasoning 字段改写为中文并重新输出 JSON]' });
                }
            }
            if (m.role === 'tool' && typeof m.content === 'string') {
                ...
                if (m.content.includes('--- [File:') && m.content.length > 500) {
                    ...
                    return Object.assign(Object.assign({}, m), { content: `[Summary: 已读取 ${filePath} 的 ${lineRange} 行代码，内容已被处理以节省 Token。如需重新查看请再次调用 code-surgeon__read_file_lines]` });
                }
                ...
                if (m.content.length > truncateLimit) {
                    return Object.assign(Object.assign({}, m), { content: m.content.substring(0, summaryLimit) + "\n...[此段历史工具输出由于过长已被折叠。核心业务逻辑已在阶段知识库中同步，若需重读全文请再次调用工具]" });
                }
            }
            if (m.role === 'assistant' && typeof m.content === 'string' && m.content.length > 600) {
                return Object.assign(Object.assign({}, m), { content: m.content.substring(0, 220) + "\n...[中间过程已压缩，保留关键上下文以节省 Token]" });
            }
            ...
            if (isInCodePhase && m.role === 'assistant' && typeof m.content === 'string' && m.content.length > 2000 && i < messages.length - 15) {
                return Object.assign(Object.assign({}, m), { content: m.content.substring(0, 500) + "\n...[早期分析内容已压缩以腾出 Token 空间，核心结论已由系统在阶段知识库中备份]" });
            }
            return m;
        });
    }
     ...
     * 获取工具结果的截断保护文本
     */
    protectToolResult(rawName, content) {
