...
class ToolGatekeeper {
    ...
    isWriteTool(toolName) {
        const lowerName = toolName.toLowerCase();
        return (lowerName.includes('apply_patch') ||
     ...
     * 检查是否正在调用读取或探测相关的工具
     */
    isReadOrProbeTool(toolName) {
        const lowerName = toolName.toLowerCase();
        return (lowerName.includes('read_file_lines') ||
     ...
     */
    isExplorationTool(toolName) {
        const lowerName = toolName.toLowerCase();
        return (this.isReadOrProbeTool(toolName) ||
     ...
     */
    checkReadLoop(toolName, args) {
        if (!this.isReadOrProbeTool(toolName))
            return null;
...
- **禁止继续对此文件执行探测尝试。**`;
     ...
     * 生成针对文档未完成的门禁提示信息
     */
    getInterceptionNotice(undoneDocTasks) {
...
5. 完成所有文档任务并获取摘要结论后，对应的代码手术刀工具才会解锁。`;
     ...
     */
    getPatchRetryNotice(toolResultContent, args) {
...
exports.ToolGatekeeper = ToolGatekeeper;
