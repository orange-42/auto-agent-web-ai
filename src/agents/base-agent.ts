...
import { recordRunTokenUsage } from "../run-token-ledger";
import {
  AgentExecutionPolicy,
  buildReadLoopGuardConfig,
  ReadLoopGuardConfig,
} from "./execution-guard-policy";
import {
  analyzeRepairGaps,
  classifyRepairWriteCategory,
  detectRepairStagnation,
  RepairGapAnalysis,
  RepairStagnationState,
  RepairWriteCategory,
} from "./repair-gap-analyzer";
import { buildRepairInstruction, RepairInstructionDecision } from "./repair-instruction-builder";
import { extractUiAnchorHints, UiAnchorHint } from "./ui-anchor-hints";
 ...
 * 所有阶段 Agent 的共同基类。
 *
 * BaseAgent 负责的不是“某个阶段的业务理解”，而是统一执行框架：
 * 1. 流式调用模型并解析增量响应
 * 2. 管理工具调用与工具结果回填
 * 3. 管理强制收敛、写码守卫、空响应恢复、循环拦截
 * 4. 记录 agent_rounds / llm usage / 原始流量日志
 * 5. 对写入行为做 AST / 项目契约 / import 现实性校验
 *
 * 各子类只需要关心：
 * - 组织本阶段 system/user prompt
 * - 选择允许使用的工具
 * - 指定本阶段的交付门槛（例如某些落地阶段要求真实写入后才能结束）
 */
export abstract class BaseAgent {
  ...
  // 某些落地型 agent 会在 execute() 中动态填充这些目标，
  // BaseAgent 再据此判断“是否允许结束本轮”。
  protected requiredWriteTargets: string[] = [];
  protected requiredSubstantiveWriteTargets: string[] = [];
  protected requiredRuntimeLinkTargets: string[] = [];
  // 编码阶段会把目标组件快照挂到这里，供 repair analyzer 提炼通用 UI 锚点。
  protected currentTargetComponentContext: string = "";
  ...
  protected toNonNegativeTokenCount(value: unknown): number {
  ...
  protected extractUsageFromStreamPayload(payload: any): {
    promptTokens: number;
    ...
    const promptTokens = this.toNonNegativeTokenCount(usage.prompt_tokens);
    const completionTokens = this.toNonNegativeTokenCount(usage.completion_tokens);
    const totalTokens = this.toNonNegativeTokenCount(usage.total_tokens);
    const cachedPromptTokens = this.toNonNegativeTokenCount(usage.prompt_tokens_details?.cached_tokens);
    const reasoningTokens = this.toNonNegativeTokenCount(usage.completion_tokens_details?.reasoning_tokens);

    // provider 可能只回部分 usage 字段。
    // 这里只要任意一个核心字段存在，就保留这次 usage 快照；
    // 后面会在同一轮内取“信息最完整 / total_tokens 最大”的那份。
    if (
      promptTokens === 0 &&
      completionTokens === 0 &&
      totalTokens === 0 &&
  ...
  protected pickPreferredUsageSnapshot(
    current: {
      promptTokens: number;
      completionTokens: number;
      totalTokens: number;
    ...
    // 同一轮流式响应里，usage 往往只会在最后一个 chunk 出现一次。
    // 这里额外做一个稳妥合并：优先保留 total_tokens 更大的快照，
    // 避免被中间态或字段更少的 usage 覆盖掉。
    ...
    if (next.totalTokens === current.totalTokens) {
      ...
      if (nextSignal >= currentSignal) return next;
    }
    return current;
  }
  ...
  protected formatToolResultProgress(toolName: string, args: any, toolRes: string): string | null {
    ...
    if (
      ...
      return this.usesForceWriteRecovery()
    ...
    if (
      ...
      if (toolName.includes("internal_structured_edit") && operation === "upsert_import") {
        return `[系统] 已补充依赖导入：${basename}。`;
      }
      if (toolName.includes("internal_structured_edit") && operation === "ensure_export") {
        return `[系统] 已补充导出声明：${basename}。`;
      }
  ...
  /**
   * 各个具体 Agent 通过覆写这个策略对象来声明自己的阶段特性。
   *
   * 这样 BaseAgent 只消费“阶段能力”，而不是硬编码认识某几个具体类名。
   * 后续如果新增一个阶段型 Agent，只需要在子类里声明策略，不必回头修改基类分支。
   */
  protected getExecutionPolicy(): AgentExecutionPolicy {
    return {};
  }

  protected getPhaseIndex(): number {
    const displayPhase = this.getExecutionPolicy().displayPhase;
    if (displayPhase) return DISPLAY_PHASES.indexOf(displayPhase);
    return 0;
  }

  protected getExplorationBudget(): number | null {
    return this.getExecutionPolicy().explorationBudget ?? null;
  }

  protected requiresWriteBeforeFinish(): boolean {
    return Boolean(this.getExecutionPolicy().requiresRealWriteBeforeFinish);
  }

  protected getPromptCharacterBudget(): number | null {
    return this.getExecutionPolicy().promptCharacterBudget ?? null;
  }

  protected usesForceWriteRecovery(): boolean {
    return Boolean(this.getExecutionPolicy().forceWriteRecovery);
  }

  protected usesForceConclusionOnExplorationSaturation(): boolean {
    return Boolean(this.getExecutionPolicy().forceConclusionOnExplorationSaturation);
  }

  protected getReadLoopGuardConfig(): ReadLoopGuardConfig {
    return buildReadLoopGuardConfig(this.getExecutionPolicy());
  }

  protected getForceWriteExplorationBudget(): number {
    return Math.max(1, this.getExecutionPolicy().forceWriteExplorationBudget ?? 3);
  }

  protected setTargetComponentContext(context: string) {
    this.currentTargetComponentContext = String(context || "");
  }

  protected isForceWriteRecoveryExplorationTool(toolName: string): boolean {
    return (
      toolName.includes("read_file_lines") ||
      toolName.includes("get_file_outline") ||
      toolName.includes("read_text_file") ||
      toolName.includes("list_directory")
    );
  }

  protected isAllowedDuringForceWriteRecovery(toolName: string): boolean {
    return (
      toolName === "internal_structured_edit" ||
      toolName === "internal_surgical_edit" ||
      toolName === "surgical_edit" ||
      this.isForceWriteRecoveryExplorationTool(toolName)
    );
  }
  ...
  protected getMissingRequiredSubstantiveWriteTargets(successfulSubstantiveWritePaths: Set<string>): string[] {
    if (!this.requiresWriteBeforeFinish()) return [];

    const normalizedRequired = this.requiredSubstantiveWriteTargets
      .filter(Boolean)
      .map((item) => this.resolveProjectFilePath(item));

    return normalizedRequired
      .filter((item) => !successfulSubstantiveWritePaths.has(item))
      .map((item) => `${item}（需完成非导入级实质修改）`);
  }

  protected getMissingDeliveryWriteTargets(
    successfulWritePaths: Set<string>,
    successfulSubstantiveWritePaths: Set<string> = successfulWritePaths,
    successfulRuntimeLinkWritePaths: Set<string> = successfulSubstantiveWritePaths,
  ): string[] {
    return Array.from(
      new Set([
        ...this.getMissingRequiredWriteTargets(successfulWritePaths),
        ...this.getMissingRequiredSubstantiveWriteTargets(successfulSubstantiveWritePaths),
        ...this.getMissingRequiredRuntimeLinkTargets(successfulRuntimeLinkWritePaths),
      ]),
    );
  }

  protected hasSatisfiedRequiredWrites(
    successfulWritePaths: Set<string>,
    successfulSubstantiveWritePaths: Set<string> = successfulWritePaths,
    successfulRuntimeLinkWritePaths: Set<string> = successfulSubstantiveWritePaths,
  ): boolean {
    return this.requiresWriteBeforeFinish()
      && this.getMissingDeliveryWriteTargets(
        successfulWritePaths,
        successfulSubstantiveWritePaths,
        successfulRuntimeLinkWritePaths,
      ).length === 0;
  }

  protected getMissingRequiredRuntimeLinkTargets(successfulRuntimeLinkWritePaths: Set<string>): string[] {
    if (!this.requiresWriteBeforeFinish()) return [];

    const normalizedRequired = this.requiredRuntimeLinkTargets
      .filter(Boolean)
      .map((item) => this.resolveProjectFilePath(item));

    return normalizedRequired
      .filter((item) => !successfulRuntimeLinkWritePaths.has(item))
      .map((item) => `${item}（需补齐与功能落点直接相关的脚本/模板接线）`);
  }
  ...
  protected isSubstantiveWriteOperation(
    toolName: string,
    args: Record<string, unknown> | undefined,
    writeKind: "create" | "modify" | null,
  ): boolean {
    if (writeKind === "create") return true;
    if (writeKind !== "modify") return false;

    if (toolName === "internal_surgical_edit" || toolName === "surgical_edit") {
      return true;
    }

    if (toolName !== "internal_structured_edit") return true;

    const operation = String(args?.operation || "");
    return !["upsert_import", "ensure_export"].includes(operation);
  }

  protected extractTemplateRelevantContent(text: unknown): string {
    return String(text || "")
  ...
  protected containsTemplateBindingMarkers(...segments: Array<unknown>): boolean {
    ...
    const templateText = this.extractTemplateRelevantContent(rawText);
    const searchableText = templateText || rawText;
    if (
      /\{\{[\s\S]*?\}\}/u.test(searchableText) ||
      /\b(?:v-if|v-else|v-else-if|v-show|v-for|v-model|v-bind|v-on|slot-scope|#\w+)\b/u.test(searchableText) ||
      /@[A-Za-z][\w:-]*/u.test(searchableText) ||
      /:[A-Za-z][\w-]*/u.test(searchableText)
    ) {
      return true;
    }

    const tagSource = templateText || rawText;
    return /<(?!\/?(?:script|style)\b)[A-Za-z][\w-]*(?:\s[^>]*)?>/u.test(tagSource);
  }

  protected templateBindingChanged(beforeText: unknown, afterText: unknown): boolean {
    const beforeTemplate = this.extractTemplateRelevantContent(beforeText);
    const afterTemplate = this.extractTemplateRelevantContent(afterText);
    if (!beforeTemplate && !afterTemplate) return false;
    if (beforeTemplate === afterTemplate) return false;
    return this.containsTemplateBindingMarkers(afterTemplate || beforeTemplate);
  }

  protected isTemplateScopedAnchor(anchor: unknown): boolean {
    const rawText = String(anchor || "").trim();
    if (!rawText) return false;
    const templateText = this.extractTemplateRelevantContent(rawText) || rawText;
    return (
      /<\/template>|<template\b/u.test(templateText) ||
      /<(?!\/?(?:script|style)\b)[A-Za-z][\w-]*(?:\s[^>]*)?>/u.test(templateText) ||
      /<\/(?!script|style)[A-Za-z][\w-]*>/u.test(templateText)
    );
  }

  protected isRuntimeLinkWriteOperation(
    toolName: string,
    args: Record<string, unknown> | undefined,
    writeKind: "create" | "modify" | null,
    resolvedPath: string,
    ...
    if (writeKind === "create") {
      return !/\.vue$/i.test(resolvedPath) || this.containsTemplateBindingMarkers(args?.content);
    }
    if (writeKind !== "modify") return false;

    if (toolName === "internal_surgical_edit" || toolName === "surgical_edit") {
      if (!/\.vue$/i.test(resolvedPath)) return true;
      return this.templateBindingChanged(args?.search, args?.replace);
    }

    if (toolName !== "internal_structured_edit") return false;

    const operation = String(args?.operation || "");
    if (["upsert_import", "ensure_export"].includes(operation)) return false;

    if (!/\.vue$/i.test(resolvedPath)) return true;

    if (operation === "ensure_vue_option_entry") {
      return false;
    }

    if (["insert_before_anchor", "insert_after_anchor", "replace_range_by_lines"].includes(operation)) {
      return this.containsTemplateBindingMarkers(args?.content) ||
        (String(args?.content || "").trim().length > 0 && this.isTemplateScopedAnchor(args?.anchor));
    }

    if (operation === "update_object_property") return false;
  ...
  protected buildPostWriteReviewInstruction(missingTargets: string[] = []): string {
    const remainingLine = missingTargets.length > 0
      ? `当前仍缺少真实写入：${missingTargets.join("、")}。`
      : "如果目标组件仍缺少真实落点、关键状态承接、交互方法、初始化链路或 API 调用闭环，请先补齐再结束。";

    return [
      "你刚完成了一次真实写入，先不要急着输出最终 JSON。",
      "下一轮请先基于已写入结果做一次局部复查，用 State(状态) / Logic(逻辑) / View(视图) 作为检查框架，判断当前任务真正需要补齐哪些层。",
      "重点检查：真实 UI 落点、data、methods、computed、watch、生命周期、API 调用链、状态展示与点击交互是否已经按当前任务要求落地。",
      remainingLine,
      "严禁在源码中写入 `// ...`、`/* ... */`、`<!-- ... -->`、`existing code`、`省略其余代码` 等占位式省略内容；修改局部代码时必须保留原文件未改部分的真实源码。",
      "只有确认不再缺少实际落点后，才能输出最终 JSON。",
    ].join("\n");
  }
  ...
  protected isCodeImportTarget(resolvedPath: string): boolean {
  ...
  protected validateImportStatementAgainstProject(
    ...
    try {
      if (this.isCodeImportTarget(resolved)) {
  ...
  /**
   * 对完整文件内容做一次“全量 import 现实性校验”。
   *
   * 之前我们只在 upsert_import / create_file 的局部路径上做校验，
   * 但模型也会通过 insert_after_anchor 这类操作把整行 import 直接塞进文件，
   * 这样就绕开了局部校验，最终写出类似 `config/feature-status` 这种项目里根本不存在的模块。
   *
   * 这里改成对“变更后的整份文件”逐条扫描 import，确保：
   * 1. 本地 alias / 相对路径真的能解析
   * 2. 默认导入 / 命名导入和目标模块导出形态一致
   */
  protected validateFileImportsAgainstProject(
    sourceRelativePath: string,
    content: string,
    options: { allowMissingRelative?: boolean } = {},
  ): string | null {
    const importStatements = content.match(/^\s*import\s+.+?\s+from\s+['"][^'"]+['"]\s*;?\s*$/gm) || [];
    for (const statement of importStatements) {
      const validationError = this.validateImportStatementAgainstProject(sourceRelativePath, statement, options);
  ...
  /**
   * 提取 Vue SFC 的 `<script>` 内容，供项目契约校验复用。
   *
   * 我们只想校验脚本区，不想被 template/style 中的文本误伤。
   */
  protected extractVueScriptContent(content: string): string {
   ...
   * 收集当前文件是否违反“项目硬约束”。
   *
   * 这层和 AST 不同，它不是看语法通不通，而是看：
   * - import 是否真实存在
   * - Vue2 Options API 组件有没有被写成 Vue3 / 模块级脚本拼贴风格
   *
   * 返回数组而不是单条字符串，是为了在 mutation 场景下做“前后 diff”，
   * 只拦截这次写入新增的问题，不阻塞对历史脏文件的渐进修复。
   */
  protected collectProjectContractViolations(
    rawPath: string,
    content: string,
    options: { allowMissingRelative?: boolean; previousContent?: string } = {},
  ): string[] {
    const violations: string[] = [];
    const importValidationError = this.validateFileImportsAgainstProject(rawPath, content, {
      allowMissingRelative: options.allowMissingRelative,
    });
    if (importValidationError) violations.push(importValidationError);

    const vueContractError = this.validateVueOptionsApiContract(
      rawPath,
      options.previousContent || "",
      content,
    );
    if (vueContractError) violations.push(vueContractError);

    return violations;
  }

  /**
   * Vue2 Options API 组件结构契约校验。
   *
   * 这次 run 的核心问题不是 AST 崩掉，而是“语法没错，但结构完全错位”：
   * - 在 import 区后面直接塞 `let featureState = null`
   * - 在 `export default` 外声明 `async function handleFeatureAction()`
   * - 在 Options API 组件里混入 Vue3/Composition API 风格的写法
   *
   * 这些代码 TypeScript 可能照样能 parse，但在真实 Vue2 组件里是明显的错误落点。
   * 所以这里补一层项目语义契约，只在“本次写入新增了这些问题”时拦截。
   */
  protected validateVueOptionsApiContract(
    rawPath: string,
    previousContent: string,
    nextContent: string,
  ): string | null {
    if (!/\.vue$/i.test(String(rawPath || ""))) return null;

    const nextScript = this.extractVueScriptContent(nextContent);
    if (!nextScript) return null;

    const usesOptionsApi =
      /export\s+default\s*\{/.test(nextScript) &&
      (/\bdata\s*\(\)\s*\{/.test(nextScript) || /\bmethods\s*:\s*\{/.test(nextScript));
    if (!usesOptionsApi) return null;

    const previousScript = this.extractVueScriptContent(previousContent);
    const [, nextPrelude = ""] = nextScript.split(/export\s+default\s*\{/);
    const [preExportBlock = ""] = nextScript.split(/export\s+default\s*\{/);
    const [previousPreExportBlock = ""] = previousScript.split(/export\s+default\s*\{/);

    const stripPreludeNoise = (value: string) =>
      value
        .split(/\r?\n/)
        .filter((line) => {
    ...
    const currentPrelude = stripPreludeNoise(preExportBlock);
    const previousPrelude = stripPreludeNoise(previousPreExportBlock);

    if (/\bthis\./.test(currentPrelude) && !/\bthis\./.test(previousPrelude)) {
      return "❌ 错误: 检测到在 Vue 组件 `export default` 之外新增了依赖 `this` 的脚本逻辑。当前项目以 Vue2 Options API 为主，实例方法必须写入 `methods`，生命周期必须写入根级 `mounted/created`，不要在 import 区后直接声明函数。";
    }

    if (/(?:^|\n)\s*(?:let|var)\s+[A-Za-z_$][\w$]*/.test(currentPrelude) && !/(?:^|\n)\s*(?:let|var)\s+[A-Za-z_$][\w$]*/.test(previousPrelude)) {
      return "❌ 错误: 检测到在 Vue 组件 `export default` 之外新增了模块级可变状态 (`let/var`)。当前目标组件是 Vue2 Options API，组件状态必须收敛到 `data()` 返回值中。";
    }

    if (/(?:^|\n)\s*(?:async\s+)?function\s+[A-Za-z_$][\w$]*\s*\(/.test(currentPrelude) && !/(?:^|\n)\s*(?:async\s+)?function\s+[A-Za-z_$][\w$]*\s*\(/.test(previousPrelude)) {
      return "❌ 错误: 检测到在 Vue 组件 `export default` 之外新增了顶层函数声明。若这是组件交互/接口方法，请改写到 `methods`；若这是初始化逻辑，请通过 `ensure_vue_option_entry(section: \"root\")` 补到 `mounted/created`。";
    }

    if (/(?:^|\n)\s*(?:async\s+)?(?:mounted|created|beforeMount|beforeDestroy|destroyed)\s*\(/.test(currentPrelude)) {
      return "❌ 错误: 检测到把 Vue 生命周期钩子写在了 `export default` 外部。当前项目是 Vue2 Options API，生命周期必须作为组件根级选项存在，不能以游离函数/片段形式插在 import 区附近。";
    }

    const compositionApiPattern = /\b(?:setup\s*\(|defineComponent\s*\(|reactive\s*\(|ref\s*\(|onMounted\s*\(|watchEffect\s*\()/;
    if (
      compositionApiPattern.test(nextScript) &&
      !compositionApiPattern.test(previousScript) &&
      !/from\s+['"]@vue\/composition-api['"]|from\s+['"]vue['"]/.test(previousScript)
    ) {
      return "❌ 错误: 检测到向 Vue2 Options API 组件中新增了 `setup/ref/reactive/onMounted` 等 Composition API 风格写法。当前目标项目以 Options API 为主，请继续使用 `data / computed / methods / mounted` 完成集成。";
    }
  ...
  protected validateMutationWithAstGuard(rawPath: string, beforeContent: string, afterContent: string): string | null {
    ...
    if (after.diagnostics.length > 0) {
      if (before.diagnostics.length > 0 && after.diagnostics.length <= before.diagnostics.length) {
        return null;
      }
      return `❌ 错误: 本次写入会引入 AST/语法错误：${this.formatAstDiagnostics(rawPath, afterContent)}。请改用更小的结构化修改或修正逗号/括号/对象项边界后再重试。`;
    }

    // 对“语法没错但结构错位”的情况做前后 diff，
    // 只拦截这次写入新增的项目契约问题，避免被历史脏文件长期锁死。
    const beforeViolations = new Set(
      this.collectProjectContractViolations(rawPath, beforeContent, {
        previousContent: beforeContent,
      }),
    );
    const afterViolations = this.collectProjectContractViolations(rawPath, afterContent, {
      previousContent: beforeContent,
    });
    const newViolation = afterViolations.find((item) => !beforeViolations.has(item));
    if (newViolation) return newViolation;
  ...
  protected validateNewContentWithAstGuard(rawPath: string, content: string): string | null {
    ...
    // 这里继续补一层项目契约校验，尤其拦截：
    // - 臆造 import
    // - Vue2 组件被写成 Vue3/模块级脚本风格
    const contractViolations = this.collectProjectContractViolations(rawPath, content, {
      allowMissingRelative: true,
    });
    if (contractViolations.length > 0) {
      return contractViolations[0];
    }
  ...
  protected buildSearchRetryInstruction(relativePath: string, snippet: string): string {
    ...
    return (
      `你已经进入编码修复聚焦模式。请停止无边界扫描，但仍可进行少量必要补读以确认真实上下文。\n` +
      `本轮只允许两类行为：\n1. 最多 ${this.getForceWriteExplorationBudget()} 次最小必要补读（code-surgeon:read_file_lines / code-surgeon:get_file_outline / filesystem:read_text_file / filesystem:list_directory）\n2. 尽快使用 internal_structured_edit 或 internal_surgical_edit 写入目标组件\n` +
      `当前问题文件：${relativePath}\n` +
      snippetBlock +
  ...
  protected buildEmptyResponseRecoveryInstruction(
    ...
    if (this.requiresWriteBeforeFinish()) {
      ...
      return (
        ...
        `2. 如果确实还差关键上下文，只允许少量必要补读（最多 ${this.getForceWriteExplorationBudget()} 次），然后立刻写入。\n` +
  ...
  protected ensureCommaBeforeInsertion(content: string, insertAt: number): { content: string; insertAt: number } {
    ...
    const previousLine = content.slice(previousLineStart, cursor + 1);
    const lineCommentIndex = previousLine.indexOf("//");
    const blockCommentIndex = previousLine.indexOf("/*");
    const trailingCommentIndex =
      lineCommentIndex >= 0 && blockCommentIndex >= 0
        ? Math.min(lineCommentIndex, blockCommentIndex)
        : Math.max(lineCommentIndex, blockCommentIndex);
    const effectiveLine = trailingCommentIndex >= 0
      ? previousLine.slice(0, trailingCommentIndex)
      : previousLine;
    const trimmedEffectiveLine = effectiveLine.replace(/\s+$/, "");
    if (!trimmedEffectiveLine) return { content, insertAt };

    const prevChar = trimmedEffectiveLine[trimmedEffectiveLine.length - 1];
    if (prevChar === "{" || prevChar === "[" || prevChar === ",") {
      return { content, insertAt };
    }
    const commaInsertAt = previousLineStart + trimmedEffectiveLine.length;
    const nextContent = `${content.slice(0, commaInsertAt)},${content.slice(commaInsertAt)}`;
    return {
      content: nextContent,
      insertAt: insertAt + 1,
    };
  ...
  protected buildSyntheticCompletionResult(
    ...
    successfulSubstantiveWritePaths: Set<string>,
    successfulRuntimeLinkWritePaths: Set<string>,
  ): any | null {
    ...
    const missingWriteTargets = this.getMissingDeliveryWriteTargets(
      successfulWritePaths,
      successfulSubstantiveWritePaths,
      successfulRuntimeLinkWritePaths,
    );
    ...
    return {
      reasoning: summarizeText(cleaned),
      ...
      completion_summary: summarizeText(cleaned),
   ...
   * 可以把它理解成一个“带工具的受控状态机”：
   * - 先向模型发起流式请求
   * - 解析 content / reasoning / tool_calls
   * - 如有工具调用，则执行工具并把结果塞回消息上下文
   * - 如已产出 JSON，则尝试结束
   * - 如出现空响应、只推理不行动、重复探索、重复 patch，则触发保护和恢复
   *
   * 绝大多数 run 的“为什么停住 / 为什么重试 / 为什么被强制收敛”，都能在这里找到原因。
   */
  public async callLLM(
    ...
    // 内建工具由 BaseAgent 直接实现，目的是把“最终写盘能力”收束在受控边界内，
    // 避免完全依赖外部 MCP 工具时缺少项目契约校验。
    ...
    const getEffectiveTools = (
      ...
      if (failureCount > 0) {
        const coreTools = currentTools.filter(
            ...
            t.function.name.includes("list_directory") ||
            t.function.name.includes("directory_tree"),
        );
    ...
    const successfulSubstantiveWritePaths = new Set<string>();
    const successfulRuntimeLinkWritePaths = new Set<string>();
    ...
    let forceWriteWarningCount = 0;
    let readLoopSaturationCount = 0;
    let explorationSaturationCount = 0;
    const recentExactReadSnippets = new Map<string, string>();
    const searchMissCounts = new Map<string, number>();
    const recentRepairGapSignatures: string[] = [];
    const recentWriteCategories: RepairWriteCategory[] = [];
    const pushLimitedHistory = <T>(target: T[], value: T, limit: number = 4) => {
      target.push(value);
      while (target.length > limit) target.shift();
    };

    // 整个 agent 生命周期由多个 round 组成；
    // 每个 round 的目标只有一个：要么继续调用工具推进，要么合法收敛成最终 JSON。
    for (let round = 0; round < roundLimit; round++) {
      ...
      let roundRequestStatus = "success";
      ...
      try {
        // 这里会根据当前状态动态收缩工具范围：
        // - forceConclusionMode: 禁止再调工具，只允许交付
        // - forceWriteMode: 进入“修复聚焦模式”，保留编辑工具 + 少量必要补读
          ...
          : forceWriteMode
	            ? (getEffectiveTools(tools, consecutiveFailureCount) || []).filter(
                    ...
                    t.function.name === "internal_structured_edit" ||
	                  t.function.name === "internal_surgical_edit" ||
                    (!forceEditOnlyMode &&
                      !this.hasSatisfiedRequiredWrites(
                        successfulWritePaths,
                        successfulSubstantiveWritePaths,
                        successfulRuntimeLinkWritePaths,
                      ) &&
                      this.isForceWriteRecoveryExplorationTool(t.function.name)),
              )
        ...
        this.traceRound({
          ...
          forceConclusionMode,
          forceWriteMode,
          repairFocusMode: forceWriteMode,
          forceEditOnlyMode,
        });
        ...
        for await (const chunk of response.data) {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              roundUsageSnapshot = this.pickPreferredUsageSnapshot(
                roundUsageSnapshot,
                this.extractUsageFromStreamPayload(data),
              );
        ...
        if (!forceConclusionMode && toolCalls.length === 0) {
          ...
          if (heuristicToolCalls.length > 0) {
            ...
            this.traceRound({
              ...
              reasoningPreview: summarizeText(this.stripPseudoToolMarkup(fullReasoning)),
            });
          }
        }
        ...
        if (hasTools) {
          ...
          let sawSuccessfulWriteThisRound = false;
          ...
          const buildRepairGuidance = (
            ...
            instruction: RepairInstructionDecision;
            uiAnchorHints: UiAnchorHint[];
          } => {
            const gapAnalysis = analyzeRepairGaps(missingTargets);
            const stagnationState = detectRepairStagnation({
              recentGapSignatures: recentRepairGapSignatures,
              currentAnalysis: gapAnalysis,
              recentWriteCategories,
            });
            const uiAnchorHints =
              stagnationState.runtimeLinkPending && this.currentTargetComponentContext.trim()
                ? extractUiAnchorHints(this.currentTargetComponentContext)
                : [];

            this.traceRound({
              type: "repair_gap_analysis",
              round,
              gapType: gapAnalysis.gapType,
              gapSignature: gapAnalysis.gapSignature,
              missingWriteTargets: gapAnalysis.missingTargets,
              runtimeLinkPending: stagnationState.runtimeLinkPending,
              stagnating: stagnationState.isStagnating,
              repeatedRounds: stagnationState.repeatedRounds,
            });
            if (gapAnalysis.gapSignature && gapAnalysis.hasBlockingGap) {
              pushLimitedHistory(recentRepairGapSignatures, gapAnalysis.gapSignature);
            }

            if (uiAnchorHints.length > 0) {
              this.traceRound({
                type: "ui_anchor_hints_generated",
                round,
                hintCount: uiAnchorHints.length,
                hintKinds: uiAnchorHints.map((hint) => hint.kind),
              });
            }

            if (stagnationState.isStagnating) {
              this.traceRound({
                type: "repair_stagnation_detected",
                round,
                gapType: gapAnalysis.gapType,
                repeatedRounds: stagnationState.repeatedRounds,
                repeatedScriptOnlyWrites: stagnationState.repeatedScriptOnlyWrites,
              });
            }

            const targetPath =
              gapAnalysis.buckets.runtimeLink[0] ||
              gapAnalysis.buckets.substantive[0] ||
              gapAnalysis.buckets.fileWrite[0] ||
              missingTargets[0] ||
              "";
            const instruction = buildRepairInstruction({
              ...
              targetPath,
              uiAnchorHints,
              forceWriteExplorationBudget: this.getForceWriteExplorationBudget(),
              fallbackInstruction,
            });

            if (instruction.upgraded) {
              this.traceRound({
                type: "repair_instruction_upgraded",
                round,
          ...
          // 本轮所有工具调用按顺序执行；
          // 每个工具结果都会继续反哺到 currentMessages，形成下一轮上下文。
            ...
            const readLoopNotice = this.toolGatekeeper.checkReadLoop(
              fullName,
              args,
              this.getReadLoopGuardConfig(),
            );
            ...
            // 写码类工具成功后，会在这里累计真实写入痕迹。
            // 后面的“是否允许结束”完全依赖这些落盘记录，而不是只看模型自述。
	            if (
	              forceWriteMode &&
	              !this.isAllowedDuringForceWriteRecovery(fullName)
	            ) {
              toolRes = "❌ 错误: 已进入编码修复聚焦模式。请停止继续扩张探索范围，只保留少量必要补读并尽快回到真实写入。";
            } else if (
              forceWriteMode &&
              this.isForceWriteRecoveryExplorationTool(fullName) &&
              ++forceWriteRecoveryReads > this.getForceWriteExplorationBudget()
            ) {
              forceEditOnlyMode = true;
              toolRes = `❌ 错误: 编码修复聚焦模式下，最小必要补读次数已达上限（${this.getForceWriteExplorationBudget()}）。请立即调用 internal_structured_edit 或 internal_surgical_edit 写入目标文件。`;
            } else if (
              ...
              if (this.usesForceWriteRecovery()) {
                forceWriteMode = true;
              ...
              if (this.usesForceConclusionOnExplorationSaturation()) {
                forceConclusionMode = true;
                forceConclusionReason = "已触发同文件高频读取拦截，现有证据已经足够，需要直接输出实施方案。";
                triggeredForceConclusionThisRound = true;
              } else if (this.usesForceWriteRecovery()) {
                readLoopSaturationCount++;
                if (readLoopSaturationCount >= 2 || forceWriteWarningCount >= 1) {
                  forceWriteMode = true;
                  forceWriteReason = "编码阶段已多次对同一核心文件进行高频读取，需要停止无边界扫描并开始收敛修复。";
                  forceEditOnlyMode = false;
                  forceWriteRecoveryReads = 0;
                  triggeredForceWriteThisRound = true;
                } else {
                  forceWriteWarningCount++;
                  toolRes += "\n\n[系统提示] 当前检测到高频重复读取，但尚未进入修复聚焦模式。请减少重复扫描，优先基于现有证据或做少量必要补读后继续落代码。";
                }
              }
            } else if (
...
禁止继续调用 search/read/outline/list/directory_tree 类工具。`;
              if (this.usesForceConclusionOnExplorationSaturation()) {
                ...
                explorationSaturationCount++;
                if (explorationSaturationCount >= 2 || forceWriteWarningCount >= 1) {
                  forceWriteMode = true;
                  forceWriteReason = `编码阶段探索次数已多次超过预算 ${explorationBudget}，需要停止继续扩张上下文并尽快进入收敛修复。`;
                  forceEditOnlyMode = false;
                  forceWriteRecoveryReads = 0;
                  triggeredForceWriteThisRound = true;
                } else {
                  forceWriteWarningCount++;
                  toolRes += "\n\n[系统提示] 当前探索次数已接近收敛阈值，但你仍可保留少量必要读取来确认真实上下文。请优先回到核心文件完成实现，不要继续泛化搜索。";
                }
              }
            ...
            this.traceRound({
              ...
              tool: fullName,
                        ...
                        responsePreview: summarizeText(toolRes),
                      });
                      currentMessages.push({ role: "tool", tool_call_id: call.id, name: call.function.name, content: toolRes });
                      continue;
                    }
                    ...
                    if (targetExists) {
                      ...
                      if (!existingTrimmed) {
                        ...
                        if (this.usesForceWriteRecovery()) {
                          ...
                          forceWriteInstruction = this.buildSearchRetryInstruction(relativeTargetPath, summarizeText(existingContent));
                          forceWriteRecoveryReads = 0;
                          triggeredForceWriteThisRound = true;
                        }
                      }
              ...
              if (
                ...
                if (__res && __res.content && Array.isArray(__res.content)) {
                  ...
                  const protectedToolResult = this.contextManager.protectToolResult(fullName, toolRes, {
                    path: typeof args?.path === "string" ? String(args.path) : "",
                  });
	                toolRes = protectedToolResult.content;
                  if (protectedToolResult.decision.applied) {
                    this.traceRound({
                      type: "tool_result_protection_applied",
                      round,
                      tool: fullName,
                      strategy: protectedToolResult.decision.strategy,
                      path: protectedToolResult.decision.path,
                      originalChars: protectedToolResult.decision.originalChars,
                      finalChars: protectedToolResult.decision.finalChars,
                      note: protectedToolResult.decision.note,
                    });
                  }
	              } catch (err: any) {
            ...
            if (
              ...
              const isSubstantiveWrite = this.isSubstantiveWriteOperation(fullName, args, writeKind);
              const isRuntimeLinkWrite = this.isRuntimeLinkWriteOperation(fullName, args, writeKind, resolvedPath);
              if (isSubstantiveWrite) {
                successfulSubstantiveWritePaths.add(resolvedPath);
              }
              if (isRuntimeLinkWrite) {
                successfulRuntimeLinkWritePaths.add(resolvedPath);
              }
              pushLimitedHistory(
                recentWriteCategories,
                classifyRepairWriteCategory({
                  writeKind,
                  isRuntimeLink: isRuntimeLinkWrite,
                  isSubstantive: isSubstantiveWrite,
                  isNoop: false,
                }),
              );
              sawSuccessfulWriteThisRound = true;

              const recoveredPendingCreateFailure = pendingCreateFailures.size === 0 &&
                /新建文件|空文件|尚未创建成功|重复 create_file/u.test(forceWriteReason);
              if (recoveredPendingCreateFailure) {
              ...
              const missingWriteTargetsAfterThisWrite = this.getMissingDeliveryWriteTargets(
                successfulWritePaths,
                successfulSubstantiveWritePaths,
                successfulRuntimeLinkWritePaths,
              );
              if (this.usesForceWriteRecovery() && !forceConclusionMode && !sawNoopEditThisRound) {
                toolRes += `\n\n⚠️【写后复查】：局部写入已落盘，请立即对照计划与目标组件复查 State(状态) / Logic(逻辑) / View(视图) 是否已经全部闭环。` +
                  (missingWriteTargetsAfterThisWrite.length > 0
                    ? ` 当前仍缺少：${missingWriteTargetsAfterThisWrite.join("、")}。`
                    : " 尤其检查 template/UI 绑定、事件触发、显隐状态和接口调用链是否已经真实落地；若还有遗漏，严禁输出最终 JSON，必须立刻继续修改。");
              }
            }
            if (
              writeKind === null &&
          ...
          // 只有当 requiredWriteTargets / substantive / runtime-link 都满足时，
          // 当前阶段才具备“可以收口”的资格。
          if (
            ...
            // 解除“防卫过当”：不要因为刚完成了一次成功写入就打断模型。
            // 只要没有退化成重复/无效修改，就允许它继续把 template / methods / 生命周期补齐。
            if (sawNoopEditThisRound) {
              forceConclusionMode = true;
              forceConclusionReason = "关键目标文件已经写入完成，后续操作已退化为重复/无效修改，必须立即输出最终 JSON 交付结果。";
              forceEditOnlyMode = true;
              triggeredForceConclusionThisRound = true;
          ...
          if (triggeredForceConclusionThisRound) {
            ...
            const repairGuidance = buildRepairGuidance(
              missingWriteTargets,
              forceWriteInstruction || this.buildSearchRetryInstruction("核心组件", ""),
                ...
                legacyType: "forced_write",
	            });
	            currentMessages.push({
	              role: "user",
	              content:
                  `${repairGuidance.instruction.content}\n` +
	                `\n触发原因：${forceWriteReason}`,
	            });
          }
          if (sawSuccessfulWriteThisRound && !triggeredForceConclusionThisRound && this.requiresWriteBeforeFinish()) {
            const missingWriteTargets = this.getMissingDeliveryWriteTargets(
              successfulWritePaths,
              successfulSubstantiveWritePaths,
              successfulRuntimeLinkWritePaths,
            ...
            const repairGuidance = buildRepairGuidance(
              missingWriteTargets,
              this.buildPostWriteReviewInstruction(missingWriteTargets),
            );
            this.traceRound({
              type: "post_write_review",
              round,
              missingWriteTargets,
              gapType: repairGuidance.gapAnalysis.gapType,
              stagnating: repairGuidance.stagnationState.isStagnating,
            });
            currentMessages.push({
              role: "user",
              content: repairGuidance.instruction.content,
            });
          }
          continue;
        }
        ...
        if (hasSearchReplace || hasJson) {
          ...
          if (hasJson) {
            ...
            this.traceRound({
              ...
              thoughtStreamed: didStreamThought,
              jsonKeys: Object.keys(parsedJson || {}),
              reasoningPreview: summarizeText(this.extractVisibleReasoning(parsedJson)),
            });
        ...
        if (synthesizedCompletion) {
          ...
          this.traceRound({
            ...
            reasoningPreview: summarizeText(synthesizedCompletion?.reasoning || ""),
          });
        ...
        if (hasReasoning && !hasTools) {
          ...
          const compactReasoning = this.isNoisyReasoningChunk(visibleReasoning)
            ? ""
            : summarizeText(visibleReasoning);
          ...
          this.traceRound({
            ...
            reasoningPreview: summarizeText(contentToPush),
          });
        ...
        roundRequestStatus = "error";
        roundErrorMessage = String(err.message || "");
        ...
        // 这里在 finally 中统一记账，确保“成功返回 / continue 重试 / 抛错中断”
        // 三条路径都会留下本轮 LLM 消耗记录，不会只统计成功的调用。
        recordRunTokenUsage({
          runId: this.runId,
          agent: this.constructor.name,
          round,
          model: this.config.model || this.config.modelId || "",
          durationMs: Date.now() - roundStartedAt,
          status: roundRequestStatus,
          finishReasons: Array.from(finishReasons).filter(Boolean),
          promptTokens: roundUsageSnapshot?.promptTokens,
          completionTokens: roundUsageSnapshot?.completionTokens,
          totalTokens: roundUsageSnapshot?.totalTokens,
          cachedPromptTokens: roundUsageSnapshot?.cachedPromptTokens,
          reasoningTokens: roundUsageSnapshot?.reasoningTokens,
          hasUsage: Boolean(roundUsageSnapshot),
        });
        this.traceRound({
          type: "llm_usage",
          round,
          ...
          finishReasons: Array.from(finishReasons).filter(Boolean),
          hasUsage: Boolean(roundUsageSnapshot),
          promptTokens: roundUsageSnapshot?.promptTokens || 0,
          completionTokens: roundUsageSnapshot?.completionTokens || 0,
          totalTokens: roundUsageSnapshot?.totalTokens || 0,
          cachedPromptTokens: roundUsageSnapshot?.cachedPromptTokens || 0,
          reasoningTokens: roundUsageSnapshot?.reasoningTokens || 0,
          errorPreview: roundErrorMessage ? summarizeText(roundErrorMessage) : "",
        });
      }
    }
