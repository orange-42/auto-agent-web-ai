...
/**
 * PRDAgent 负责把原始需求文档压缩成“编码前能直接消费的业务事实”。
 *
 * 它关注的不是技术实现，而是：
 * - 业务规则
 * - UI 落点
...
export class PRDAgent extends BaseAgent {
  ...
  /**
   * execute 的输入通常是预抓取后的 PRD 正文和高优先级证据锚点。
   * 这一步尽量把“模糊需求”收敛成结构化 artifact，供 API / PLAN / CODING 复用。
   */
...
${lessons}
    ...
    - 如果文档给出了多个页面或模块，请明确区分“全局入口”“详情页入口”“上下游流程联动”“数据记录字段”等不同落点。
