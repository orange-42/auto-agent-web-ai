...
/**
 * QAAgent 负责浏览器侧的轻量自动化验证。
 *
 * 它只做少量高价值检查，不承担“探索整站”的职责。
 * 真正的本地静态校验、runtime 启动、环境探测在 orchestrator 中完成；
 * QAAgent 拿到的是一个已经尽量准备好的测试站点与验证点集合。
 */
export class QAAgent extends BaseAgent {
  protected getExecutionPolicy() {
    return {
      displayPhase: "验证测试",
      explorationBudget: 5,
      promptCharacterBudget: 10000,
    };
  }

  /**
   * execute 输入的是“已经收敛好的测试目标”，包括：
   * - baseUrl
   * - 关键验证点
   * - 结构化 test cases
   * - 最近代码改动摘要
