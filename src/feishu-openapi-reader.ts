import axios, { AxiosInstance } from "axios";

export interface PrefetchedSheetContext {
  token: string;
  title: string;
  preview: string;
  diagnostics: string[];
}

export interface PrefetchedSourceContext {
  url: string;
  status: "success" | "partial" | "error";
  resolvedType?: string;
  content: string;
  diagnostics: string[];
  sheetContexts: PrefetchedSheetContext[];
}

interface OpenApiEnvelope<T> {
  code?: number;
  msg?: string;
  data?: T;
}

interface WikiNodePayload {
  node?: {
    obj_token?: string;
    obj_type?: string;
    title?: string;
    space_id?: string;
    parent_node_token?: string;
    has_child?: boolean;
    node_token?: string;
  };
}

interface WikiNodesPayload {
  items?: Array<{
    node_token?: string;
    obj_token?: string;
    obj_type?: string;
    title?: string;
    has_child?: boolean;
  }>;
}

interface DocumentMetadataPayload {
  document?: {
    document_id?: string;
    title?: string;
  };
}

interface DocumentRawContentPayload {
  content?: string;
}

interface DocumentBlockPayload {
  block?: {
    block_type?: number;
    sheet?: {
      token?: string;
    };
  };
}

type GenericRecord = Record<string, unknown>;

export class FeishuOpenApiReader {
  private readonly accessToken: string;
  private readonly baseUrl: string;
  private readonly client: AxiosInstance | null;

  constructor() {
    this.accessToken =
      process.env.FEISHU_USER_ACCESS_TOKEN ||
      process.env.LARK_USER_ACCESS_TOKEN ||
      process.env.FEISHU_ACCESS_TOKEN ||
      process.env.LARK_ACCESS_TOKEN ||
      "";
    this.baseUrl = (process.env.FEISHU_OPEN_BASE_URL || process.env.LARK_OPEN_BASE_URL || "https://open.feishu.cn").replace(/\/+$/, "");
    this.client = this.accessToken
      ? axios.create({
          baseURL: this.baseUrl,
          timeout: 20_000,
          headers: {
            Authorization: `Bearer ${this.accessToken}`,
            "Content-Type": "application/json",
          },
        })
      : null;
  }

  isConfigured(): boolean {
    return Boolean(this.client);
  }

  configurationDiagnostics(): string[] {
    if (this.client) return [];
    return [
      "未检测到可直连飞书 OpenAPI 的用户级 access token。",
      "可通过环境变量提供 `FEISHU_USER_ACCESS_TOKEN`（或 `LARK_USER_ACCESS_TOKEN`），让项目内直连读取替代 lark-cli 子进程。",
    ];
  }

  async readSource(url: string, signal?: AbortSignal): Promise<PrefetchedSourceContext> {
    if (!this.client) {
      return {
        url,
        status: "error",
        content: "飞书 OpenAPI 直连未启用：缺少用户级 access token。",
        diagnostics: this.configurationDiagnostics(),
        sheetContexts: [],
      };
    }

    try {
      if (this.isWikiUrl(url)) return await this.readWikiSource(url, signal);
      if (this.isDocUrl(url)) return await this.readDocumentSource(url, undefined, signal);
      if (this.isSheetUrl(url)) return await this.readSheetSource(url, undefined, signal);

      return {
        url,
        status: "error",
        content: "当前链接不是可识别的飞书 wiki/docx/sheets 链接。",
        diagnostics: ["仅支持飞书 wiki/docx/doc/sheets 预读。"],
        sheetContexts: [],
      };
    } catch (error: any) {
      return {
        url,
        status: "error",
        content: `飞书 OpenAPI 直连失败：${this.humanizeError(error)}`,
        diagnostics: [this.humanizeError(error)],
        sheetContexts: [],
      };
    }
  }

  private async readWikiSource(url: string, signal?: AbortSignal): Promise<PrefetchedSourceContext> {
    const wikiToken = this.extractToken(url, /\/wiki\/([^/?#]+)/);
    const diagnostics: string[] = [];
    if (!wikiToken) {
      return {
        url,
        status: "error",
        content: "未能从 wiki 链接中解析 token。",
        diagnostics: ["wiki token 解析失败。"],
        sheetContexts: [],
      };
    }

    const envelope = await this.get<WikiNodePayload>("/open-apis/wiki/v2/spaces/get_node", { token: wikiToken }, signal);
    const node = envelope.data?.node;
    const resolvedType = typeof node?.obj_type === "string" ? node.obj_type : undefined;
    const objToken = typeof node?.obj_token === "string" ? node.obj_token : "";
    const spaceId = node?.space_id || "";
    const hasChild = node?.has_child || false;

    diagnostics.push(`已通过 OpenAPI 解析 wiki 节点 (Type: ${resolvedType || "unknown"}).`);

    let childrenText = "";
    if (hasChild && spaceId) {
      try {
        const childrenEnv = await this.get<WikiNodesPayload>(
          `/open-apis/wiki/v2/spaces/${spaceId}/nodes/${wikiToken}/children`,
          undefined,
          signal
        );
        const children = childrenEnv.data?.items || [];
        if (children.length > 0) {
          childrenText = "\n\n### 🚀 发现子文档 (探索路径)\n" +
            "当前 Wiki 页面包含以下子节点，如果正文内容不完整，请优先阅读这些子文档：\n" +
            children.map(c => `- [${c.title}](https://himo-group.feishu.cn/wiki/${c.node_token}) (Type: ${c.obj_type})`).join("\n");
          diagnostics.push(`发现 ${children.length} 个子节点并已注入上下文。`);
        }
      } catch (e) {
        diagnostics.push(`尝试读取子节点失败: ${this.humanizeError(e)}`);
      }
    }

    if (!objToken) {
      return {
        url,
        status: "success",
        resolvedType,
        content: `# Wiki 目录: ${node?.title || "未命名"}\n\n${childrenText || "当前是空节点或目录节点。"}`,
        diagnostics,
        sheetContexts: [],
      };
    }

    if (resolvedType === "sheet") {
      const sheet = await this.readSpreadsheetToken(objToken, signal);
      return {
        url,
        status: sheet.preview.startsWith("未能") ? "partial" : "success",
        resolvedType,
        content: sheet.preview + childrenText,
        diagnostics: diagnostics.concat(sheet.diagnostics),
        sheetContexts: [sheet],
      };
    }

    const docResult = await this.readDocumentSource(url, objToken, signal);
    docResult.resolvedType = resolvedType;
    docResult.content = docResult.content + childrenText;
    docResult.diagnostics = diagnostics.concat(docResult.diagnostics);
    return docResult;
  }

  private async readDocumentSource(url: string, providedToken?: string, signal?: AbortSignal): Promise<PrefetchedSourceContext> {
    const diagnostics: string[] = [];
    const sheetContexts: PrefetchedSheetContext[] = [];
    const documentToken = providedToken || this.extractToken(url, /\/(?:docx|doc)\/([^/?#]+)/);

    if (!documentToken) {
      return {
        url,
        status: "error",
        content: "未能从文档链接中解析 doc token。",
        diagnostics: ["doc token 解析失败。"],
        sheetContexts,
      };
    }

    const metadata = await this.get<DocumentMetadataPayload>(`/open-apis/docx/v1/documents/${documentToken}`, undefined, signal);
    const raw = await this.get<DocumentRawContentPayload>(`/open-apis/docx/v1/documents/${documentToken}/raw_content`, undefined, signal);
    const title = String(metadata.data?.document?.title || "").trim();
    const rawContent = String(raw.data?.content || "").trim();

    diagnostics.push("已通过 OpenAPI 读取文档元信息和 raw_content。");

    if (!rawContent) {
      return {
        url,
        status: "partial",
        content: title ? `# ${title}\n\n(raw_content 为空)` : "(raw_content 为空)",
        diagnostics,
        sheetContexts,
      };
    }

    const content = [title ? `# ${title}` : "", rawContent].filter(Boolean).join("\n\n");
    diagnostics.push("当前直连路径优先保证正文稳定读取；若需要文档中的内嵌 sheet 展开，可继续走 CLI/MCP 降级补读。");

    return {
      url,
      status: "partial",
      content,
      diagnostics,
      sheetContexts,
    };
  }

  private async readSheetSource(url: string, providedToken?: string, signal?: AbortSignal): Promise<PrefetchedSourceContext> {
    const spreadsheetToken = providedToken || this.extractToken(url, /\/sheets\/([^/?#]+)/);
    if (!spreadsheetToken) {
      return {
        url,
        status: "error",
        content: "未能从表格链接中解析 spreadsheet token。",
        diagnostics: ["spreadsheet token 解析失败。"],
        sheetContexts: [],
      };
    }

    const sheet = await this.readSpreadsheetToken(spreadsheetToken, signal);
    return {
      url,
      status: sheet.preview.startsWith("未能") ? "partial" : "success",
      resolvedType: "sheet",
      content: sheet.preview,
      diagnostics: sheet.diagnostics,
      sheetContexts: [sheet],
    };
  }

  private async readSpreadsheetToken(spreadsheetToken: string, signal?: AbortSignal): Promise<PrefetchedSheetContext> {
    const diagnostics: string[] = [];
    const resolved = await this.resolveEmbeddedSheetToken(spreadsheetToken, signal);
    diagnostics.push(...resolved.diagnostics);
    const effectiveToken = resolved.token;

    try {
      const metadata = await this.get<GenericRecord>(`/open-apis/sheets/v3/spreadsheets/${effectiveToken}/sheets/query`, undefined, signal);
      const sheets = this.extractSheets(metadata.data);
      
      diagnostics.push(`已通过 OpenAPI 读取表格元信息，共发现 ${sheets.length} 个子表。`);
      
      const sheetResults: PrefetchedSheetContext[] = [];
      // 预读前 2 个子表的内容作为预览，其余仅列出名称
      for (let i = 0; i < Math.min(sheets.length, 2); i++) {
        const s = sheets[i];
        const sTitle = String(s.title || s.name || s.sheet_id || `Sheet${i+1}`);
        const sRange = `${sTitle}!A1:Z20`;
        try {
          const vEnv = await this.get<GenericRecord>(
            `/open-apis/sheets/v2/spreadsheets/${effectiveToken}/values/${encodeURIComponent(sRange)}`,
            undefined, signal
          );
          const v = this.extractValues(vEnv.data);
          sheetResults.push({
            token: effectiveToken,
            title: sTitle,
            preview: this.valuesToMarkdown(sTitle, v),
            diagnostics: []
          });
        } catch (e) {
          diagnostics.push(`读取子表 ${sTitle} 失败: ${this.humanizeError(e)}`);
        }
      }

      const allSheetNames = sheets.map(s => String(s.title || s.name || "未命名")).join(", ");
      const combinedPreview = [
        `### 表格概览 (Spreadsheet Overview)`,
        `当前表格共有 ${sheets.length} 个子表: [${allSheetNames}]`,
        `以下展示前 ${sheetResults.length} 个子表的预览数据：`,
        ...sheetResults.map(sr => sr.preview)
      ].join("\n\n");

      return {
        token: spreadsheetToken,
        title: String(sheets[0]?.title || sheets[0]?.name || "Spreadsheet"),
        preview: combinedPreview,
        diagnostics,
      };
    } catch (error: any) {
      diagnostics.push(this.humanizeError(error));
      return {
        token: spreadsheetToken,
        title: "unknown",
        preview: "未能通过 OpenAPI 展开内嵌表格。",
        diagnostics,
      };
    }
  }

  private async resolveEmbeddedSheetToken(
    spreadsheetToken: string,
    signal?: AbortSignal
  ): Promise<{ token: string; diagnostics: string[] }> {
    if (!spreadsheetToken.includes("_")) {
      return { token: spreadsheetToken, diagnostics: [] };
    }

    const [docToken, blockId] = spreadsheetToken.split("_");
    if (!docToken || !blockId || docToken.length !== 27) {
      return { token: spreadsheetToken, diagnostics: [] };
    }

    try {
      const block = await this.get<DocumentBlockPayload>(
        `/open-apis/docx/v1/documents/${docToken}/blocks/${blockId}`,
        undefined,
        signal
      );
      const realToken = block.data?.block?.sheet?.token;
      if (typeof realToken === "string" && realToken.trim()) {
        return {
          token: realToken,
          diagnostics: ["已将文档内嵌 sheet block 解析为真实 spreadsheet token。"],
        };
      }
    } catch (error: any) {
      return {
        token: spreadsheetToken,
        diagnostics: [`解析内嵌 sheet block 失败：${this.humanizeError(error)}`],
      };
    }

    return {
      token: spreadsheetToken,
      diagnostics: ["未能从 docx block 响应中提取真实 spreadsheet token。"],
    };
  }

  private async get<T>(pathname: string, params?: Record<string, string>, signal?: AbortSignal): Promise<OpenApiEnvelope<T>> {
    if (!this.client) throw new Error("未配置飞书 OpenAPI access token。");
    const response = await this.client.get<OpenApiEnvelope<T>>(pathname, { params, signal });
    if (response.data?.code !== 0) {
      throw new Error(response.data?.msg || `OpenAPI 返回 code=${String(response.data?.code ?? "unknown")}`);
    }
    return response.data;
  }

  private extractToken(url: string, pattern: RegExp): string | undefined {
    const match = url.match(pattern);
    return match?.[1];
  }

  private isWikiUrl(url: string): boolean {
    return /https?:\/\/[^/]+\/wiki\//.test(url);
  }

  private isDocUrl(url: string): boolean {
    return /https?:\/\/[^/]+\/(?:docx|doc)\//.test(url);
  }

  private isSheetUrl(url: string): boolean {
    return /https?:\/\/[^/]+\/sheets\//.test(url);
  }

  private extractSheets(payload: unknown): Array<Record<string, unknown>> {
    if (!payload || typeof payload !== "object") return [];
    const data = payload as Record<string, unknown>;
    const candidates = [
      data.sheets,
      data.sheet_metadatas,
      (data.spreadsheet as Record<string, unknown> | undefined)?.sheets,
      (data.items as unknown[] | undefined),
    ];
    for (const candidate of candidates) {
      if (Array.isArray(candidate)) {
        return candidate.filter(item => Boolean(item) && typeof item === "object") as Array<Record<string, unknown>>;
      }
    }
    return [];
  }

  private extractValues(payload: unknown): unknown[][] {
    if (!payload || typeof payload !== "object") return [];
    const data = payload as Record<string, unknown>;
    const directCandidates = [
      data.values,
      (data.valueRange as Record<string, unknown> | undefined)?.values,
      (data.value_range as Record<string, unknown> | undefined)?.values,
      ((data.data as Record<string, unknown> | undefined)?.values),
      (((data.data as Record<string, unknown> | undefined)?.valueRange as Record<string, unknown> | undefined)?.values),
    ];
    for (const candidate of directCandidates) {
      if (Array.isArray(candidate)) return candidate as unknown[][];
    }
    return [];
  }

  private valuesToMarkdown(title: string, values: unknown[][]): string {
    const lines = [`## Sheet: ${title}`, ""];
    if (!values.length) {
      lines.push("- 当前 sheet 未返回单元格内容。");
      return lines.join("\n");
    }

    for (const row of values.slice(0, 20)) {
      if (!Array.isArray(row)) continue;
      const rowText = row.map(cell => String(cell ?? "").trim()).join(" | ").trim();
      if (rowText) lines.push(`- ${rowText}`);
    }

    if (lines.length === 2) lines.push("- 当前 sheet 首屏没有可展示的非空单元格。");
    return lines.join("\n");
  }

  private humanizeError(error: any): string {
    if (axios.isAxiosError(error)) {
      const code = error.response?.data?.code;
      const msg = error.response?.data?.msg || error.message;
      return code ? `${msg} (code=${code})` : msg;
    }
    return error instanceof Error ? error.message : String(error);
  }
}
