...
export interface PlanApiCoverageArtifact {
  endpoint: string;
  method: string;
  purpose: string;
  decision: string;
...
export interface PlanArtifact {
  ...
  api_coverage: PlanApiCoverageArtifact[];
...
function normalizePlanApiCoverage(input: unknown): PlanApiCoverageArtifact[] {
  if (!Array.isArray(input)) return [];

  return input
    .map((item) => {
      if (!item || typeof item !== "object") return null;
      const endpoint = String((item as any).endpoint || "").trim();
      const method = String((item as any).method || "").trim().toUpperCase();
      const purpose = String((item as any).purpose || "").trim();
      const decision = String((item as any).decision || "").trim() || "implement";
      const reason = String((item as any).reason || "").trim();
      const targetFiles = toStringArray((item as any).target_files || (item as any).targetFiles, 6);

      if (!endpoint && !purpose) return null;
      return {
        endpoint,
        method,
        purpose,
    ...
    .filter(Boolean) as PlanApiCoverageArtifact[];
}
...
export function normalizePlanArtifact(input: any): PlanArtifact {
  ...
  const apiCoverage = normalizePlanApiCoverage(input?.api_coverage);
  ...
  return {
    ...
    api_coverage: apiCoverage.slice(0, 8),
